...
from napari_neuron_navigator.logging_utils import (
...
def _reset_plugin_logger() -> None:
    ...
    for attr in (
        "_napari_neuron_navigator_debug_configured",
        "_napari_neuron_navigator_log_path",
    ):
...
def _cleanup_plugin_logger(monkeypatch):
    ...
    monkeypatch.delenv("NAPARI_NEURON_NAVIGATOR_DEBUG", raising=False)
    monkeypatch.delenv("NAPARI_NEURON_NAVIGATOR_LOG_FILE", raising=False)
...
def test_configure_debug_logging_is_noop_when_disabled() -> None:
    ...
    napari_logger = logging.getLogger(_NAPARI_SLICER_LOGGER_NAME)
    assert all(
        handler.name != "napari_neuron_navigator_debug_file"
...
def test_configure_debug_logging_adds_file_and_stream_handlers(
    ...
    log_file = tmp_path / "neuron-navigator.log"
    monkeypatch.setenv("NAPARI_NEURON_NAVIGATOR_DEBUG", "1")
    monkeypatch.setenv("NAPARI_NEURON_NAVIGATOR_LOG_FILE", str(log_file))
    ...
    assert [handler.name for handler in logger.handlers] == [
        "napari_neuron_navigator_debug_file",
        "napari_neuron_navigator_debug_stream",
    ]
    napari_logger = logging.getLogger(_NAPARI_SLICER_LOGGER_NAME)
    ...
    assert all(
        handler.name != "napari_neuron_navigator_debug_stream"
        for handler in napari_logger.handlers
    )
...
def test_configure_debug_logging_honors_custom_log_path(
    ...
    monkeypatch.setenv("NAPARI_NEURON_NAVIGATOR_DEBUG", "true")
    monkeypatch.setenv("NAPARI_NEURON_NAVIGATOR_LOG_FILE", str(nested))
...
def test_configure_debug_logging_accepts_legacy_environment_names(
    monkeypatch,
    tmp_path: Path,
) -> None:
    """Existing launchers should continue to enable logging after the rename."""
    legacy_log = tmp_path / "legacy.log"
    monkeypatch.setenv("NAPARI_SWC_VIEWER_DEBUG", "1")
    monkeypatch.setenv("NAPARI_SWC_VIEWER_LOG_FILE", str(legacy_log))

    assert configure_debug_logging() == legacy_log
...
def test_startup_timing_logs_start_and_ok_records(caplog) -> None:
    ...
    logger = logging.getLogger("napari_neuron_navigator.tests.startup_timing")
...
def test_startup_timing_logs_exception_with_exc_info(caplog) -> None:
    ...
    logger = logging.getLogger("napari_neuron_navigator.tests.startup_timing_error")
...
def test_startup_timing_is_quiet_when_debug_disabled(caplog) -> None:
    ...
    logger = logging.getLogger("napari_neuron_navigator.tests.startup_timing_disabled")
